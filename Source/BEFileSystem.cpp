...
BEValueListWideStringUniquePtr list_files_in_directory ( const filesystem::path& directory, const long file_type_wanted = kBE_FileTypeAll, const bool recurse = false, const bool include_hidden = true )
{
	...
	try {
		...
		filesystem::recursive_directory_iterator it ( directory_path, filesystem::directory_options::follow_directory_symlink );
...
const fmx::errcode write_to_file ( const filesystem::path& new_file, const vector<char>& contents, const ios_base::openmode flags )
{
	fmx::errcode error = kNoError;

	try {

		const auto parent_directory = new_file.parent_path ();
		if ( ! exists ( parent_directory ) ) {
			filesystem::create_directories ( parent_directory );
		}

		const ios_base::openmode mode = ios_base::out | flags;
		ofstream output_file ( new_file, mode );
		if ( output_file ) {
...
const uintmax_t file_or_directory_size ( const filesystem::path& path )
{
	uintmax_t size = 0;
	...
	for ( size_t i = 0 ;  i < list_of_files->size() ; ++i ) {
		...
		filesystem::path file = list_of_files->at ( i );

#ifdef FMX_WIN_TARGET
		size += file_size ( file );
#else
		if ( ! filesystem::is_symlink ( file ) ) { // boost follows ze aliases
			size += file_size ( file );
		} else {
